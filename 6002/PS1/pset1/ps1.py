...
def greedy_cow_transport(cows,limit=10):
    ...
    while len(cows_copy) != 0:
        for key in list(cows_copy.keys()):
            if cows_copy[key] <= temp_load:
                temp_load -= cows_copy[key]
                temp_trip.append(key)
                del cows_copy[key]                      
        result.append(temp_trip)
        temp_trip = []
        temp_load = limit
    end = time.time()
    print(end-start)
...
def brute_force_cow_transport(cows,limit=10):
    ...
    for i in power_list:
        ...
        if all(w <= limit for w in ship):
            possible_trips.append(i)
    unique_list=[]
    print(len(possible_trips))
    for e in possible_trips:
        if e not in unique_list:
            unique_list.append(e)
    
    min_list = min(map(len, unique_list))
